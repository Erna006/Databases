...
SELECT
    ...
    department
FROM employees
WHERE department = 'IT' AND manager_id IS NOT NULL;
...
SELECT
    ...
    ROUND(salary * 0.1, 2) as ten_percent_raise
FROM employees;
...
-- Task 4.2
-- Task 4.2: Total hours worked per project
SELECT
    p.project_id,
    p.project_name,
...
SELECT
    ...
    GREATEST(ROUND(MAX(e.salary),2), ROUND(MIN(e.salary),2)) AS greatest_value_example,
    LEAST(ROUND(MAX(e.salary),2), ROUND(MIN(e.salary),2)) AS least_value_example
FROM employees e
